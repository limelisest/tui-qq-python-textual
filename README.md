...
- `enable_message_reply_action`: 是否在消息时间后显示鼠标点击用的 `[回复]` 按钮；只影响显示，不影响键盘回复操作。
- `enable_message_plus_one_action`: 是否在消息正文后显示鼠标点击用的 `[+1]` 按钮；只影响显示，不影响键盘 `+1` 操作。
