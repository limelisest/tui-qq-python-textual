...
class PaneController:
    ...
    def pane_by_uid(self, uid: int) -> Optional[ChatPaneState]:
        return next((pane for pane in self._app.state.panes if pane.uid == uid), None)

    def active_pane(self) -> ChatPaneState:
        pane = self.pane_by_uid(self._app.state.active_pane_uid)
        if pane is not None:
            return pane
        self._app.state.active_pane_uid = self._app.state.panes[0].uid
        return self._app.state.panes[0]
    ...
    def set_input_owner_pane(
        ...
        old_uid = self._app.state.input_owner_pane_uid
        self._app.state.input_owner_pane_uid = pane.uid if pane is not None else None
    ...
    def hide_all_message_inputs(self) -> None:
        if self._app.state.input_owner_pane_uid is None:
            return
    ...
    def refresh_pane_active_classes(self) -> None:
        for pane in self._app.state.panes:
            ...
            if pane_has_active_border(
                pane,
                self._app.state.navigation.layer,
                self._app.state.active_pane_uid,
                self._app.state.navigation.top_target_pane_uid,
            ):
    ...
    def update_pane_grid_class(self) -> None:
        ...
        grid.add_class(f"pane_count_{len(self._app.state.panes)}")
        if self._app.state.split_layout_horizontal and len(self._app.state.panes) in (2, 3):
    ...
    def update_split_buttons(self) -> None:
        try:
            ...
            return
        add_btn.disabled = len(self._app.state.panes) >= MAX_SPLIT_PANES
        add_btn.tooltip = "新增分屏" if not add_btn.disabled else "最多 4 个分屏"
        try:
            layout_btn = self._app.query_one("#split_layout_btn", Button)
            layout_btn.tooltip = (
                "纵向分屏布局"
                if self._app.state.split_layout_horizontal
                else "横向分屏布局"
            )
        except NoMatches:
            pass
        close_disabled = len(self._app.state.panes) <= 1
        for pane in self._app.state.panes:
            try:
    ...
    def add_pane(self) -> None:
        if len(self._app.state.panes) >= MAX_SPLIT_PANES:
            self._app._show_toast("最多 4 个分屏")
            return
        pane = ChatPaneState(uid=self._app.state.next_pane_uid)
        self._app.state.next_pane_uid += 1
        self._app.state.panes.append(pane)
    ...
    def close_pane(self, pane: ChatPaneState) -> None:
        if len(self._app.state.panes) <= 1:
            self._app._show_toast("至少保留 1 个分屏")
            return
        if pane not in self._app.state.panes:
            return
        pane_index = self._app.state.panes.index(pane)
        self._app.state.panes.remove(pane)
        try:
            self._app.query_one(f"#chat_pane_{pane.uid}", Vertical).remove()
        except NoMatches:
            pass
        next_index = min(pane_index, len(self._app.state.panes) - 1)
        self._app.state.active_pane_uid = self._app.state.panes[next_index].uid
        if self._app.state.navigation.top_target_pane_uid == pane.uid:
            self._app.state.navigation.top_target_pane_uid = self._app.state.active_pane_uid
        if self._app.state.input_owner_pane_uid == pane.uid:
            self._app.state.input_owner_pane_uid = None
        self.update_pane_grid_class()
        self.refresh_pane_active_classes()
        self.update_pane_titles()
        self.update_split_buttons()
